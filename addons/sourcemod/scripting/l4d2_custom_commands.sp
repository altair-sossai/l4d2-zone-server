...
public OnPluginStart()
{
	...
	//Cvars
	CreateConVar("l4d2_custom_commands_version", GETVERSION, "Version of Custom Admin Commands Plugin", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	g_cvarRadius = CreateConVar("l4d2_custom_commands_explosion_radius", "350", "Radius for the Create Explosion's command explosion", FCVAR_PLUGIN);
	g_cvarPower = CreateConVar("l4d2_custom_commands_explosion_power", "350", "Power of the Create Explosion's command explosion", FCVAR_PLUGIN);
	g_cvarDuration = CreateConVar("l4d2_custom_commands_explosion_duration", "15", "Duration of the Create Explosion's command explosion fire trace", FCVAR_PLUGIN);
	g_cvarRainDur = CreateConVar("l4d2_custom_commands_rain_duration", "10", "Time out for the gnome's rain or l4d1 survivors rain", FCVAR_PLUGIN);
	g_cvarRainRadius = CreateConVar("l4d2_custom_commands_rain_radius", "300", "Maximum radius of the gnome rain or l4d1 rain. Will also affect the air strike radius", FCVAR_PLUGIN);
	g_cvarLog = CreateConVar("l4d2_custom_commands_log", "1", "Log admin actions when they use a command? [1: Yes 0: No]", FCVAR_PLUGIN);
	g_cvarAddType = CreateConVar("l4d2_custom_commands_menutype", "1", "How should the commands be added to the menu? 0: Create new category 1: Add to default categories", FCVAR_PLUGIN);
...
public Action:CmdAdren(client, args)
{
	...
	for(new i=0; i < target_count; i++)
	{
		SetAdrenalineEffect(target_list[i]);
	}
	return Plugin_Handled;
}
...
stock SetAdrenalineEffect(iTarget)
{
