...
public Plugin:myinfo = 
{
	...
	author = "djromero (SkyDavid, David Romero)",
...
public Action:cmd_spectate(client, args)
{
	...
	if (IsMapVS)
	{
		afkForceSpectate(client, false, true);
...
public Action:afkJoinHint (Handle:Timer, any:client)
{
	...
	if ((client > 0) && IsClientConnected(client) && IsClientInGame(client))
	{
		...
		if (GetClientTeam(client) == 1)
		{
			...
			PrintHintText(client, "Press M to choose a team and join the game...");
...
afkForceSpectate (client, bool:advertise, bool:self)
{
	...
	if (advertise)
	{
		...
		PrintToChatAll("\x01\x04[SM] \x03%s \x01has been switched to \x03Spectators", PlayerName);
	}
	else if (self) 	// If player switched itself ...
	{
		new String:PlayerName[200];
		GetClientName(client, PlayerName, sizeof(PlayerName));
		PrintToChatAll("\x01\x04[SM] \x03%s \x01switched himself to \x03Spectators", PlayerName);
	}
...
afkKickClient (client)
{
	...
	PrintToChatAll("\x01\x04[SM] \x03%s \x01has been kicked for being away for too long.", PlayerName);
}
